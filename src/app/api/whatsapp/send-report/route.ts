...
async function generateReportText(userId: string, preferences: {
  ...
  if (preferences.includeAiInsights) {
    ...
    const recentEmotional = closedTrades.slice(0, 5).map((t) => t.emotionalState).filter(Boolean);
    const negativeStates = recentEmotional.filter((s) => ["FOMO", "REVENGE", "ANXIOUS"].includes(s!));
    if (negativeStates.length >= 2) {
      tips.push("🧠 You've been trading emotionally. Do an NLP session before trading today.");
    }
    ...
    if (consecutiveLosses >= 3 || (consecutiveLosses === -1 && recentResults.length >= 3)) {
      tips.push("🛑 You're on a losing streak. Consider reducing size or taking a break.");
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (isTest) {
      // Return the report text without sending (for preview)
...
// Export for use by cron endpoint
export { generateReportText, sendWhatsAppMessage };
